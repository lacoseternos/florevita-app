...
function bindPageActions(){
  ...
  if(S.page==='pdv'){
    ...
    (() => {
      ...
      // Mostra os 8 produtos mais "fresquinhos" ao focar input vazio
      // — resposta instantanea (zero busca).
      const _showRecent = () => {
        const recent = (S.products || [])
          .filter(p => p.archived !== true)
          .slice(0, 12);
        if (recent.length) renderSuggestions(recent);
      };
      pdvSearchInput.addEventListener('focus', () => {
        const q = pdvSearchInput.value.trim();
        if (!q) _showRecent();
      });
      ...
      pdvSearchInput.addEventListener('input', (e) => {
        ...
        if(!q){
          // Vazio: mostra recentes em vez de esconder (resposta visual imediata)
          _showRecent();
          _pdvCurrentResults = [];
          return;
        }
        ...
        }, 120); // Reduzido de 300 -> 120ms (busca local eh barata)
      });
  ...
  if(S.page==='relatorios'){
    ...
    if(_si){
      ...
      _si.addEventListener('input', e=>{S._orderSearch=e.target.value;import('./utils/helpers.js').then(m=>m.triggerServerOrderSearch?.(e.target.value));clearTimeout(_searchTimer);_searchTimer=setTimeout(()=>{ requestAnimationFrame(()=>{ render(); requestAnimationFrame(()=>{ const el=document.getElementById('order-search-input'); if(el){el.focus();el.setSelectionRange(el.value.length,el.value.length);} }); }); }, 550);});
  ...
  if(S.page==='pedidos'){
    ...
    // Filtro por OBSERVACAO do pedido (vale junto com a busca)
    const obsInput = document.getElementById('ped-filter-obs');
    if(obsInput){
      obsInput.addEventListener('change',e=>{S._fObs=e.target.value;S._pedPage=1;render();});
      obsInput.addEventListener('keydown',e=>{ if(e.key==='Enter'){S._fObs=e.target.value;S._pedPage=1;render();} });
    }
    // ⚡ FORCAR BUSCA NO SERVIDOR — traz pedido antigo que nao esta no cache
    {const _fs=document.getElementById('btn-ped-force-search');if(_fs)_fs.onclick=async()=>{
      const st=document.getElementById('ped-force-search-status');
      const setSt=(txt,cor)=>{ if(st){ st.textContent=txt; st.style.color=cor||'var(--muted)'; } };
      _fs.disabled=true; const txtOrig=_fs.textContent; _fs.textContent='⏳ Buscando...';
      setSt('procurando no histórico completo…');
      try{
        const { fetchOrdersAdvanced } = await import('./utils/helpers.js');
        const r = await fetchOrdersAdvanced({
          q:      S._orderSearch || '',
          notes:  S._fObs || '',
          source: S._fCanal || '',
          from:   S._fDate1 || '',
        ...
        if (r.vazio) {
          setSt('preencha a busca, a observação, o canal ou o período antes.', 'var(--red)');
        } else {
          // O render() abaixo recria a barra de filtros (some o texto de
          // status), entao o resultado vai por toast pra ela conseguir ler.
          toast(r.total
            ? `🔎 ${r.total} pedido(s) no servidor · ${r.added} novo(s) carregado(s)`
            : '🔎 Nenhum pedido encontrado no servidor com esses filtros.');
          render();
        }
      }catch(e){
        setSt('❌ falha ao buscar: '+(e?.message||'servidor indisponível'), 'var(--red)');
      }finally{
        const b=document.getElementById('btn-ped-force-search');
        if(b){ b.disabled=false; b.textContent=txtOrig; }
      }
    };}
    ...
    const clearFilters=()=>{S._fStatus='Todos';S._fBairro='';S._fTurno='';S._fUnidade='';S._fCanal='';S._fObs='';S._fPagamento='';S._fPrioridade='';S._fTipo='';S._fDate1='';S._fDate2='';S._orderSearch=''; render();};
  ...
  // ── Cupons ───────────────────────────────────────────────────
  if(S.page==='cupons'){
    try{ bindCuponsEvents(); }catch(e){ console.error('bindCuponsEvents', e); }
  }
  ...
  if(S.page==='instagramDms'){
    try{ bindInstagramDms(); }catch(e){ console.error('bindInstagramDms', e); }
  }
  ...
  if (S.page === 'ecommerce' && S.user?.role === 'Administrador') {
    ...
    if (document.getElementById('ec2-accepting')) {
      (async () => {
        try {
          ...
          // Modo sempre 'loja' (catalogo removido).
          const modeEl = document.getElementById('ec2-mode');
          if (modeEl) modeEl.value = 'loja';
          _applyMode();
    ...
    (async () => {
      try {
        ...
        if (status) {
          status.innerHTML = `<span style="color:var(--muted);">No site agora: <strong>🛒 LOJA COMPLETA</strong></span>`;
        }
      } catch(_){}
    })();
